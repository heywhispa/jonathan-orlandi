...
function Stat({ target, suffix, label }: { target: number | null; suffix: string; label: string }) {
  ...
  return (
    <div className="flex-1 transition-colors duration-300" style={{ padding: "clamp(14px,3vw,20px) clamp(14px,3vw,24px)" }}
      ...
      <div className="mx-auto w-full max-w-[720px] px-5 sm:px-6">
        ...
        <RevealDiv direction="rv">
          <div className="flex items-start gap-5 sm:gap-6 mb-7">
            <div className="flex-shrink-0 relative rounded-full overflow-hidden" style={{ width: "clamp(56px,10vw,72px)", height: "clamp(56px,10vw,72px)", border: "2px solid #d4820a" }}>
              <Image src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Captura%20de%20Tela%202026-03-23%20a%CC%80s%2020.58.23-pB168oIFCOEWM5whlt4OyeUeODu3Ju.png" alt="Foto de Jonathan, psicólogo" fill className="object-cover object-top" sizes="72px" />
            </div>
            <div className="pt-1">
              ...
              <div className="font-light" style={{ fontSize: 13, color: "#9a9088", lineHeight: 1.5 }}>
                Psicólogo · Especialista em Neurociências &amp; Performance<br />TDAH diagnosticado · 383K seguidores
              </div>
            </div>
          </div>
        </RevealDiv>
        <RevealDiv direction="rv">
          <div className="flex border overflow-hidden mb-7" style={{ border: "1px solid #e2ddd6", borderRadius: 12 }}>
        ...
        <RevealDiv direction="rv">
          <div className="font-light flex flex-col gap-4" style={{ fontSize: 15, color: "#5a5248", lineHeight: 1.8 }}>
            ...
            <p>Hoje compartilho o que aprendo com <span className="inline font-medium" style={{ background: "rgba(212,130,10,.2)", borderRadius: 3, padding: "0 2px" }}>383 mil pessoas</span> que encontraram aqui o que não encontraram em mais lugar nenhum.</p>
          </div>
          <div className="font-serif font-semibold mt-6" style={{ fontSize: 14, color: "#d4820a", letterSpacing: ".05em" }}>Distraídos venceremos.</div>
        </RevealDiv>
      </div>
    </section>
  )
