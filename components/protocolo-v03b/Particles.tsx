...
export default function Particles() {
  useEffect(() => {
    const handler = (e: CustomEvent) => {
      const container = document.getElementById("pts-v03b")
      if (!container) return
  ...
  return (
    <>
      <div id="pts-v03b" style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 9999, overflow: "hidden" }} />
      <style>{`@keyframes ptfly { 0% { transform: translate(0,0) scale(1); opacity:1; } 100% { transform: translate(var(--tx),var(--ty)) scale(0); opacity:0; } }`}</style>
    </>
  )
}
