            ...
            {phases.map((ph, i) => (
              <div key={i} style={{ borderBottom: i < phases.length - 1 ? "1px solid #e2ddd6" : "none" }}>
                <button
                  ...
                  style={{ background: "none", border: "none", padding: "clamp(18px,3vw,26px) clamp(16px,4vw,28px)", cursor: "pointer" }}
                  onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(212,130,10,.03)")}
                  onMouseLeave={(e) => (e.currentTarget.style.background = "none")}
                >
                  <span className="font-serif font-bold flex-shrink-0" style={{ fontSize: 10, letterSpacing: ".1em", color: "#d4820a", background: "rgba(212,130,10,.05)", border: "1px solid rgba(212,130,10,.2)", padding: "4px 10px", borderRadius: 100, whiteSpace: "nowrap" }}>
                    {ph.num}
                  </span>
                  ...
                  <span
                    ...
                    style={{ width: 28, height: 28, border: `1px solid ${open === i ? "#d4820a" : "#e2ddd6"}`, color: open === i ? "#d4820a" : "#9a9088", fontSize: 18, transform: open === i ? "rotate(45deg)" : "none", transition: "transform .35s, border-color .2s, color .2s" }}
                  >
                    +
                  </span>
                </button>
                <div style={{ maxHeight: open === i ? 600 : 0, overflow: "hidden", transition: "max-height .5s cubic-bezier(.4,0,.2,1)" }}>
                  <div style={{ padding: "0 clamp(16px,4vw,28px) 24px", borderTop: "1px solid #e2ddd6" }}>
                    <p className="font-light" style={{ fontSize: 15, color: "#5a5248", lineHeight: 1.75, marginBottom: 16, paddingTop: 20 }}>{ph.body}</p>
                    {ph.quote && (
                      <p style={{ fontStyle: "italic", color: "#1a1612", fontSize: 15, borderLeft: "2px solid #d4820a", paddingLeft: 16, marginBottom: 16 }}>{ph.quote}</p>
                    )}
